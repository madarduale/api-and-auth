...
def create_drinks(token):
    ...
    exist_title = Drink.query.filter(Drink.title == req_title).all()
    try:
        if req_title and req_recipe:
            if exist_title:
                return (
                    jsonify(
                        {
                            "success": False,
                            "Description": "title already exist in database",
                        }
                    ),
                    422,
                )
...
def update_drinks(token, id):
    ...
    try:
        ...
        if drink:
            ...
            return (jsonify({"success": False, "Description": " request does not exist"}), 400)
